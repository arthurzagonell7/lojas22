...
window.addEventListener("load", () => {
    ...
    if (localStorage.getItem("editarProdutos")) {
        let html = document.getElementById('editarProdutos')
        let dados = JSON.parse(localStorage.getItem('editarProdutos'))

        dados.perfil === 'admin'
            ? document.getElementById('produtos_editar').style.display = 'block'
            : document.getElementById('produtos_editar').style.display = 'none'

        html.innerHTML = `<div style="display: none; background-color: red;" id="informacoes"></div>
            Perfil:${dados.perfil}
            </div>`
